...
export const projects: Project[] = [
  ...
  },
  {
    slug: "kindle-agent",
    title: "Kindle Agent",
    oneLiner: "A private, invite-only book search that sends any title straight to my family's Kindles — five sources, one tap.",
    tileInsight: "\"Send a book to a Kindle\" sounds like a search problem. It's really a formatting one — Amazon rejects most EPUBs on the internet.",
    tags: ["Consumer", "AI", "Full-Stack", "Automation"],
    thumbnailColor: "#E0A458",
    problem: "I read on a Kindle, and so do my mom and sister. Getting a book on there means a weeks-long Libby hold or Amazon's price — and Send-to-Kindle is a manual hunt-download-email loop where half the EPUBs you find fail silently.",
    hypothesis: "Put a search box in front of every free and self-hosted source, wire it into Amazon's Send-to-Kindle pipe, and the whole dance collapses to one tap. The bet: aggregation plus reliable formatting is the product, not any single source.",
    built: "An invite-only Next.js app with magic-link and password auth. Search fans out across five sources — Standard Ebooks, Gutenberg, Open Library, LibGen, and a self-hosted torrent bridge — deduped into one ranked list, then the pick is cleaned through Calibre and emailed to your Kindle via Resend.\n\nThe torrent source runs on a free Oracle VM (Jackett + qBittorrent + Calibre in Docker, over Tailscale). Built for three accounts from day one — my mom and sister each get their own library and history.",
    broke: "Amazon rejects almost every EPUB on the internet with an opaque E999 error. I burned time on AZW3 and MOBI before finding the fix: run every file through Calibre, which re-zips it to spec. Magic links kept dying to Gmail's link prefetcher, so the token consume had to go POST-only. And LibGen ranks so badly that \"the nightingale\" returns Florence Nightingale biographies.",
    learned: "The reliability layer is the product, and it's invisible until it fails. Building for three users instead of one forced the real work — uniqueness checks, send caps, and a bounce webhook so a silent delivery failure actually surfaces.",
    nextSteps: "Per-job fetch tokens so a leaked bridge token can't pull any file. Query enrichment to fix LibGen's ranking. And smoother onboarding for the one manual step every user still does themselves — allowlisting the sender on Amazon.",
    keyLearning: "The hard part isn't finding the book — it's that Amazon silently rejects most EPUBs. The real product is the Calibre formatting pipeline, not the search box in front of it.",
    services: [
      {
        layer: "Frontend & Auth",
        note: "Invite-only app, magic-link + password",
        items: ["Next.js 16 / React 19", "Tailwind CSS 4", "Magic-link + password (bcryptjs)", "Upstash Redis (users, history)"],
      },
      {
        layer: "Search Aggregation",
        note: "Five sources deduped into one ranked list",
        items: ["Standard Ebooks", "Project Gutenberg", "Open Library", "LibGen", "Self-hosted torrent bridge", "Source + seeder chips per result"],
      },
      {
        layer: "Send Pipeline",
        note: "Calibre clean → Resend → Kindle",
        items: ["Calibre EPUB reparse (the E999 fix)", "Resend (kindle@schlacter.me sender)", "Per-user Kindle address", "20/day per-user send cap"],
      },
      {
        layer: "Torrent Bridge",
        note: "Self-hosted, zero-cost, publicly reachable",
        items: ["Oracle Cloud always-free VM", "Tailscale Funnel (public HTTPS)", "Jackett + qBittorrent", "Flask + Calibre in Docker"],
      },
      {
        layer: "Multi-user Safety",
        note: "Built for three accounts from day one",
        items: ["Kindle-address uniqueness index", "Per-user send quota", "Resend bounce webhook", "Delivery test + drift banner"],
      },
    ],
    artifacts: {
      screenshots: [
        "/projects/kindle-agent/login.png",
        "/projects/kindle-agent/search-results.png",
        "/projects/kindle-agent/library.png",
      ...
      liveUrl: "https://kindle.schlacter.me",
    },
  },
  {
    slug: "ai-skills",
    title: "Claude Skills I've Built",
    oneLiner: "A gallery of the 28 Claude Code skills I designed myself — autonomous agents that maintain my sites, the system I run my job search through, a writing engine that holds my voice, and personal tools for my home and wardrobe.",
    tileInsight: "The agents that maintain my sites while I sleep are worth more than any single feature I've shipped.",
    tags: ["AI", "Automation", "Vibecoding"],
    thumbnailColor: "#C9B8E4",
    problem: "I kept solving the same problems by hand — syncing repos, fixing broken deploys, keeping my writing on-voice, capturing demos for case studies. Every repeated task was time I wasn't spending on the actual product.",
    hypothesis: "If I encoded each repeated job as a Claude Code skill, I'd stop redoing the work and start compounding it. The bet: the highest-leverage thing to build with AI is the tooling that builds everything else.",
    built: "28 skills of my own. Four run unattended — a nightly agent that pushes any unsynced code to private repos, one that fixes broken Vercel deploys straight from my inbox, a webhook that turns bug reports into shipped fixes, and an auditor that vets every third-party skill before I install it.\n\nOthers encode craft and context: five-parallel-draft code generation, a writing-voice engine, the whole system I run my job search through, a stylist that knows my closet, a design advisor that knows my house, and a weekly loop that reads my own transcripts and rewrites my instructions. Three are open-source.",
    broke: "Autonomous is the hard part. Scheduled skills fail silently — a cron that never fires looks identical to one with nothing to do — so most of the work was making failure visible: health emails, drift scans, streak counters. Skills that write to protected paths also hit sandbox walls I had to design around.",
    learned: "The skills that pay off aren't the flashy ones. They're the boring maintenance agents I never think about until one emails me. Encoding a task once and trusting it to run unattended is a different discipline than doing it well a single time.",
    nextSteps: "A public template so anyone can fork the autonomous-agent pattern. And a skill that writes skills — most of mine still start from a spec I hand-write myself.",
    keyLearning: "Encoding a job as a skill once means every future session inherits it. The maintenance agents I never think about compound faster than any single product I ship.",
    artifacts: {
      screenshots: [
        "/projects/ai-skills/overview.png",
        "/projects/ai-skills/career.png",
        "/projects/ai-skills/cards.png",
        "/projects/ai-skills/utilities.png",
      ],
      screenshotCaptions: [
      ...
      liveUrl: "https://schlacter.me/skills",
    },
  },
  {
    slug: "llm-explainer",
