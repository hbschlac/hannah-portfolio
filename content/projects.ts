...
export const projects: Project[] = [
  ...
  },
  {
    slug: "kindle-agent",
    title: "Kindle Agent",
    oneLiner: "A private, invite-only library for my family and friends: search free sources for a book or an audiobook, and get it on a Kindle or straight onto your phone.",
    ...
    problem: "I read on a Kindle, and so do my mom and my sister. Getting a book on there means a weeks-long Libby hold or Amazon's price, and Send-to-Kindle is a manual hunt-download-email loop where half the EPUBs you find fail silently. Once other people started using it, a second problem showed up: when it broke, nobody could tell me what happened. They just stopped getting books.",
    ...
    built: "An invite-only Next.js app with magic-link auth. One search fans out across five sources in parallel — Standard Ebooks, Gutenberg, Open Library, LibGen, and a self-hosted torrent bridge — deduped into a single ranked list. The pick is repaired to spec and delivered.\n\nDelivery has two modes, because not everyone reading is on a Kindle. Kindle users get the file emailed. Everyone else downloads it straight to the device, which turned out to be the harder path — an iPhone home-screen app has no download manager, so a JavaScript blob download silently does nothing. That path navigates the page to a GET route instead, and iOS then offers Open in Books or Save to Files.\n\nIt has grown well past search-and-send. Audiobooks run as a deliberately separate subsystem, since a 700MB .m4b breaks every assumption the ebook path makes. Want to Read watches for a book you couldn't get yet and delivers it the moment a copy appears. Followed Book Lists tracks what real book clubs are reading each month and tells you when a new pick lands. A shared EPUB cache means the second person to want a book gets it instantly.",
    broke: "Amazon rejects almost every EPUB on the internet with an opaque E999 error. I burned time on AZW3 and MOBI before finding the fix: rewrite the file to spec, which is the actual product. Magic links kept dying to Gmail's link prefetcher, so the token consume had to go POST-only.\n\nThe failure I didn't predict was legibility. Search would return nothing and there was no way to know whether the book didn't exist, a source was down, or the query was worded wrong. Three books failed in one day for three different reasons, and the telling one had a perfectly spelled title whose file simply has no author in the name, so including the author is what lost it. Searching one way and calling that the answer was the bug.",
    learned: "When other people depend on your side project, the work shifts from adding features to telling the truth about failures. Every empty search now records what each source actually did, and the admin page will name which LibGen mirror timed out. A search tries the title with the author, then the title alone, then the title without its subtitle, then the author alone.\n\nThe constraint I'm most glad I kept: no LLM or search API key anywhere in the product. Anything that needs open-ended reasoning runs as an MCP server on the user's own Claude, so the app stays deterministic, free to run, and never breaks because a key expired.",
    nextSteps: "Per-job fetch tokens so a leaked bridge token can't pull any file. Better onboarding for the one manual step every user still does themselves: allowlisting the sender on Amazon. And a real answer for the books no source has, which is still the honest gap.",
    keyLearning: "Amazon silently rejects most EPUBs on the internet, so the file-repair step ended up being the whole product. Once my family was depending on it, diagnosing failures mattered just as much — a break nobody can explain reads the same as a product that's broken.",
    services: [
      {
        layer: "Frontend & Auth",
        note: "Invite-only, installable as a home-screen app",
        items: ["Next.js 16 / React 19", "Tailwind CSS 4", "Magic-link auth, 30-day sessions", "Upstash Redis", "Versioned What's New splash"],
      },
      {
        layer: "Search Aggregation",
        note: "Five sources in parallel, one ranked list",
        items: ["Standard Ebooks", "Project Gutenberg", "Open Library", "LibGen", "Self-hosted torrent bridge", "Four-way query fallback on a miss"],
      },
      {
        layer: "Delivery",
        note: "Kindle email or direct to device",
        items: ["EPUB rewritten to spec (the E999 fix)", "Resend to a per-user Kindle address", "In-place GET download for iOS home-screen apps", "Shared EPUB cache for repeat books", "Per-user send cap"],
      },
      {
        layer: "Audiobooks",
        note: "A parallel subsystem, not a bolt-on",
        items: ["Download-only, a Kindle can't play one", "100MB to 1GB .m4b or bundled .mp3", "Its own queue, routes, and progress UI", "Per-device save instructions"],
      },
      {
        layer: "Discovery",
        note: "Finds books you're not searching for",
        items: ["Want to Read auto-delivers on availability", "Followed Book Lists tracks club picks", "Monthly pick notifications", "Reading status, ratings, and stats"],
      },
      {
        layer: "Reliability",
        note: "Every failure has to be answerable",
        items: ["Per-source outcome on every empty search", "Named LibGen mirror failures", "Live bridge health check", "Per-person admin drill-down", "Resend bounce webhook"],
      },
    ],
    artifacts: {
      screenshots: [
        ...
        "/projects/kindle-agent/want-to-read.png",
        "/projects/kindle-agent/book-lists.png",
      ],
      screenshotCaptions: [
        "The front door: a private, invite-only library for family and friends. A book can land on a Kindle or download straight to the device you are holding.",
        "One search fans out across every source at once, each result tagged with where it came from. The Books/Audiobooks toggle switches to a separate audiobook pipeline.",
        "Your library: covers, Goodreads ratings, reading status, and filters by genre or by the book list a title came from.",
        "Want to Read: books no source had yet, and who recommended them. The app keeps watching and delivers one the moment a copy appears.",
        "Followed Book Lists: follow a public club and its monthly picks show up here, with a notification when a new one lands.",
      ],
  ...
  },
  {
    ...
    oneLiner: "A live developer-conversation tracker for a major AI lab's Managed Agents API — scheduled scrape, categorization, and dashboard showing what early adopters are actually asking for.",
    ...
    problem: "A major AI lab shipped a Managed Agents API on April 10, 2026. I wanted a live read of what developers were saying about it — not a one-shot survey, but a standing feed a PM could open every morning and see what shifted overnight. Note: this is a concept build — the API launch and the developer feed are simulated, so the project demonstrates the listening system rather than tracking a real shipped product.",
    ...
    nextSteps: "Widen the sources beyond Reddit/HN — Discord, X, the lab's forum itself. Add a 'Shipped vs. Requested' module comparing developer asks against the lab's API changelog. Alert if a new post matches a high-priority theme (e.g., rate limits, pricing) within 10 minutes of being posted.",
    ...
    artifacts: {
      ...
      screenshotCaptions: [
        "Dark-mode dashboard: live developer pulse on a major AI lab's Managed Agents API.",
  ...
  },
  {
    slug: "ldor",
    title: "L'dor",
    oneLiner: "A modern cultural heritage brand built for the next generation.",
    ...
    problem: "I'm that 28-year-old — moving into my first real apartment, wanting to host Shabbat, and realizing that modern cultural heritage pieces basically don't exist. Everything available was either synagogue gift shop or family heirloom: dated, overpriced for institutions, or designed for someone else entirely.",
    ...
    artifacts: {
      ...
      screenshotCaptions: [
        "Landing: cultural heritage, reimagined for you — pricing anchored at $22 and $88",
